...
# --- Define a list of popular stocks for the menu ---
popular_stocks = ['AAPL', 'MSFT', 'GOOGL', 'AMZN', 'TSLA', 'META', 'NFLX', 'NVDA', 'AMD', 'JPM', 'V', 'PG', 'KO', 'PEP', 'DIS']
selected_ticker = st.sidebar.selectbox("Select a stock from the list:", popular_stocks)
ticker_symbol = st.sidebar.text_input("Or enter a different stock ticker:", selected_ticker).upper()
...
selected_model_name = st.sidebar.selectbox(
    "Select Model for Next Day Prediction:",
    ("Best Model", "Logistic Regression", "Random Forest", "Gradient Boosting", "K-Nearest Neighbors", "SVC")
)
st.sidebar.markdown("---")
...
def train_and_evaluate_models(data_with_features):
    ...
    return results, best_model_name, scaler, feature_cols
...
if __name__ == "__main__":
    ...
    if df is not None:
        ...
        if df_features is not None:
            ...
            up_count = (df_features["Target"] == 1).sum()
            down_count = (df_features["Target"] == 0).sum()
            total_count = len(df_features)
            st.subheader("Historical Stock Movement Distribution")
            st.write(f"**Total Trading Days:** {total_count}")
            st.write(f"**Days with price increase ('Up'):** {up_count} ({up_count/total_count:.2%})")
            st.write(f"**Days with price decrease/no change ('Down'):** {down_count} ({down_count/total_count:.2%})")
            st.markdown("---")
            ...
            fig_candlestick.update_layout(xaxis_rangeslider_visible=False, title=f"Candlestick Chart for {ticker_symbol.upper()}")
            ...
            with col1:
                ...
                fig_rsi = px.line(df_features, x=df_features.index, y="RSI_14", title="RSI (14-day)")
                fig_rsi.add_hline(y=70, line_dash="dash", line_color="red", annotation_text="Overbought")
                fig_rsi.add_hline(y=30, line_dash="dash", line_color="green", annotation_text="Oversold")
                st.plotly_chart(fig_rsi)
            with col2:
                st.subheader("Moving Average Convergence Divergence (MACD)")
                fig_macd = px.line(df_features, x=df_features.index, y=["MACD", "MACD_signal"], title="MACD and Signal Line")
                st.plotly_chart(fig_macd)

            # Training and evaluation
            with st.spinner("Training models..."):
                results, best_model_name, scaler, feature_cols = train_and_evaluate_models(df_features)
            ...
            if results:
                ...
                # Determine which model to use for next-day prediction
                model_to_predict = None
                prediction_model_name = ""
                if selected_model_name == "Best Model":
                    if best_model_name:
                        model_to_predict = results[best_model_name]["model"]
                        prediction_model_name = best_model_name
                        st.success(f"The best performing model is **{best_model_name}** with an accuracy of {results[best_model_name]['accuracy']:.4f}")
                else:
                    if selected_model_name in results:
                        model_to_predict = results[selected_model_name]["model"]
                        prediction_model_name = selected_model_name
                        st.info(f"Using **{prediction_model_name}** for prediction.")
                    else:
                        st.warning(f"The selected model '{selected_model_name}' could not be loaded. Please try a different model.")
                ...
                if model_to_predict:
                    ...
                    try:
                        ...
                        prediction = model_to_predict.predict(last_row_scaled)[0]
                        probability = model_to_predict.predict_proba(last_row_scaled)[0]
                        
                        st.subheader(f"Prediction for **{ticker_symbol.upper()}** tomorrow using {prediction_model_name}:")
                        if prediction == 1:
                    ...
                    try:
                        best_model_results = results[prediction_model_name]
